package com.ai.webservice.servlet;
...
public class CXFServlet extends CXFNonSpringServlet implements
	...
	@Component
	public static class ApplicationContextHold implements
	...
	protected void loadBus(ServletConfig servletConfig) {
		...
		String configLocation = servletConfig
				.getInitParameter("config-location");
	...
	protected void addListener(AbstractApplicationContext wac) {
		try {
			...
			Collection<Object> c = CastUtils
					.cast((Collection<?>) ReflectionUtil.setAccessible(m)
							.invoke(wac));
			c.add(this);
		} catch (Throwable t) {
			// ignore.
	...
	private ApplicationContext createSpringContext(ApplicationContext ctx,
			ServletConfig servletConfig, String location) {
		...
		if (ctx != null) {
			...
			if (names == null || names.length == 0) {
				ctx2.setConfigLocations(new String[] {
						"classpath:/META-INF/cxf/cxf.xml", location });
			} else {
	...
	public void onApplicationEvent(ContextRefreshedEvent event) {
		wac = event.getApplicationContext();
		destroy();
		setBus(null);
		try {
			init(getServletConfig());
		} catch (ServletException e) {
			throw new RuntimeException("Unable to reinitialize the CXFServlet",
					e);
		}
	}
